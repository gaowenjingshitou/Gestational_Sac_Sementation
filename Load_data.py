...
class Data(Dataset):
    ...
    def __init__(self, root, resize, mode):
        ...
        # 训练集
        train_data = pd.read_csv("train_data_0307.csv", converters={"Inhosp_No": str}, )
        train_data = train_data.set_index("Inhosp_No")
        print(train_data.shape)

        # 验证集
        val_data = pd.read_csv("val_data_0307.csv", converters={"Inhosp_No": str}, )
        val_data = val_data.set_index("Inhosp_No")
        print(val_data.shape)

        # 测试集
        test_data = pd.read_csv("test_data_0307.csv", converters={"Inhosp_No": str}, )
        test_data = test_data.set_index("Inhosp_No")

        # print(test_data.shape)
        # print(test_data[["img", 'label']])
